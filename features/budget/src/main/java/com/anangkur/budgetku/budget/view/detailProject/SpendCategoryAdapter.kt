...
class SpendCategoryAdapter(
...
) : BaseAdapter<ItemSpendCategoryBinding, SpendCategoryUiModel>() {
    ...
    override fun bind(data: SpendCategoryUiModel, itemView: ItemSpendCategoryBinding, position: Int) {
        itemView.apply {
            ivBudget.setImageUrl(data.image)
            tvBudget.text = data.title
            tvRemaining.text = data.remaining.toDouble().currencyFormatToRupiah()
