...
import com.anangkur.budgetku.auth.databinding.ActivityEditPasswordBinding
import com.anangkur.budgetku.base.BaseActivity
import com.anangkur.budgetku.presentation.features.auth.EditPasswordViewModel
...
class EditPasswordActivity: BaseActivity<ActivityEditPasswordBinding, EditPasswordViewModel>(),
    ...
    override val mTitleToolbar: String?
        get() = getString(authR.string.toolbar_edit_profile)
    ...
    private fun observeViewModel(){
        mViewModel.apply {
            ...
            successEditPassword.observe(this@EditPasswordActivity, Observer {
                showToastShort(getString(authR.string.message_success_edit_password))
                finish()
            })
    ...
    private fun validateInput(oldPassword: String?, newPassword: String?, confirmPassword: String?){
        when {
            oldPassword.isNullOrEmpty() -> {
                ...
                mLayout.tilPasswordOld.error = getString(authR.string.error_old_password_empty)
            }
            !oldPassword.validatePassword() -> {
                mLayout.tilPasswordOld.isErrorEnabled = true
                mLayout.tilPasswordOld.error = getString(authR.string.error_password_not_valid)
            }
            newPassword.isNullOrEmpty() -> {
                mLayout.tilPasswordNew.isErrorEnabled = true
                mLayout.tilPasswordNew.error = getString(authR.string.error_new_password_empty)
            }
            !newPassword.validatePassword() -> {
                mLayout.tilPasswordNew.isErrorEnabled = true
                mLayout.tilPasswordNew.error = getString(authR.string.error_password_not_valid)
            }
            confirmPassword.isNullOrEmpty() -> {
                mLayout.tilPasswordConfirm.isErrorEnabled = true
                mLayout.tilPasswordConfirm.error = getString(authR.string.error_confirm_password_empty)
            }
            !confirmPassword.validatePasswordConfirm(newPassword) -> {
                mLayout.tilPasswordConfirm.isErrorEnabled = true
                mLayout.tilPasswordConfirm.error = getString(authR.string.error_password_confirm_not_valid)
            }
            else -> {
